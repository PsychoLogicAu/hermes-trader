...
#
# ── Port notes (hermes-trader, 2026-09-16) ─────────────────────────────
# Hand port of uni2ts@8062ef5 model/moirai2/forecast.py. Deltas vs upstream:
#   * base class LightningModule removed (plain object; hparams -> attrs)
#   * gluonts imports removed: predict() below is the public API and never
#     touched the gluonts predictor stack (create_predictor /
#     describe_inputs / get_default_transform / prediction_input_names /
#     training_input_names were predictor-only and are dropped)
#   * CausalMeanImputation (uni2ts.transform.imputation, which imports the
#     gluonts transform base classes) replaced by the numpy-only
#     _CausalMeanImputation below — the imputation math is copied
#     verbatim from upstream transform/imputation.py
#   * jaxtyping annotations kept (no-ops at runtime)
# The numeric path (forward / predict / _convert / _format_preds /
# _generate_time_id / _patched_seq_pad) is otherwise verbatim. Fidelity is
# gated against the official uni2ts implementation (<1e-4 relative median
# divergence, 20+ real series) before merge — see scratch/moirai2_validate.py.
...
class _LastValueImputation:
    """Verbatim port of uni2ts.transform.imputation.LastValueImputation."""

    def __init__(self, value: int | float | complex = 0.0):
        self.value = value

    def __call__(self, x: Float[np.ndarray, "length *dim"]) -> Float[np.ndarray, "length *dim"]:
        x = x.T
...
class _CausalMeanImputation:
    """Verbatim port of uni2ts.transform.imputation.CausalMeanImputation.

    Replaces each missing value with the average of all the values up to
    this point, ensuring causality. Leading missing values are replaced by
    the closest non-missing value; an all-NaN sequence becomes `value`.
    """
    ...
    def __call__(self, x: Float[np.ndarray, "length *dim"]) -> Float[np.ndarray, "length *dim"]:
        ...
        last_value_imputation = _LastValueImputation(self.value)
...
class Moirai2Forecast:
    """Dependency-reduced Moirai-2 forecaster (see module docstring).

    Drop-in for the upstream ``uni2ts.model.moirai2.Moirai2Forecast`` for
    the ``predict(past_target)`` call: same signature, same numpy output
    shape ``(batch, num_quantiles, future_time)`` for univariate input.

    Usage (mirrors the MoiraiAgent project's tsf_models.py):
        module = Moirai2Module.from_pretrained("Salesforce/moirai-2.0-R-small")
        model = Moirai2Forecast(module=module, prediction_length=horizon,
                                context_length=ctx, target_dim=1,
                                feat_dynamic_real_dim=0,
                                past_feat_dynamic_real_dim=0)
        out = model.predict([[float(c) for c in closes]])  # (1, 9, horizon)
    """
    ...
    def __call__(self, *args, **kwargs):
        # Upstream inherits this from nn.Module (via LightningModule); the
        # ported class is a plain object, so delegate explicitly. predict()
        # calls self(**data_entry) exactly as upstream does.
            ...
            if data_entry[key] is not None and isinstance(data_entry[key], list):
                for idx in range(len(data_entry[key])):
                    if data_entry[key][idx].shape[0] > self.context_length:
                        ...
                        pad_length = (
                            self.context_length - data_entry[key][idx].shape[0]
                        )
                        pad_block = np.full(
                            (pad_length, 1),

...
def cmd_duel():
    ...
    if not duel_store.duelist_enabled():
        ...
        print()
        print("  Set the duelist model to enable A/B evaluation, e.g. the agent")
        print("  config's `llm` block:")
        print('    "llm": { "duelist_model": "<model id>" }')
        print("  (env fallback: LLM_DUEL_MODEL; base_url/api_key fall back to")
        print("  the primary LLM_* values)")
        print()
        return

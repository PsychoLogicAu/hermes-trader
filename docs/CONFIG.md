...
Duelist-veto conviction gate (the 19th gate). Keys off the A/B duelist — the second model (`.env.local` `LLM_DUEL_MODEL`, resolved via `duel_store.duelist_config()`) that re-answers the same research prompt and logs its verdict to `duel-logs/hermes-trader-duel.jsonl`. **Re-keyed 2026-09-03**: when the primary issues a directional entry (LONG/SHORT) and the duelist **explicitly vetoes** the setup (**VETO** — see VETO vs PASS in the system prompt) or takes the **opposite side**, the entry must clear elevated conviction: `confidence >= min_conf` (0.90) OR `composite >= min_composite` (60) — otherwise it is vetoed. A neutral **PASS no longer vetoes**: PASS was redefined as "nothing excited me" (abstention), while VETO means "this setup is a trap — avoid like the plague". Motivation: the 2026-09-03 headroom replay (32 trades since the Qwen2.5-Coder-7B anchor) found the old PASS-keyed rule blocking 27/32 entries with a blocked cohort net +$0.57 — bare abstention carries no quality signal (the duelist PASSes ~84% of everything), so flipping the old key live would have LOST money; the original 2026-08-31 replay (+$1.83 over 41 trades) predates both keys and does not transfer either. The duelist's verdict rides into the executor via the analysis dict's `duelist_at_entry` snapshot (written by research.py at entry), so the gate is a pure context read — no extra LLM call, no new I/O. Fail-safes (no-opinion pass): disabled; no duelist verdict (duelist disabled or the second call failed); duelist agrees with the primary's side; duelist PASS; unrecognised verdict. A data gap can never block a trade. **SHADOW by default**: `shadow_mode: true` (and how it ships) makes it structurally pass and the executor logs `[gate][SHADOW] duelist_veto WOULD HAVE BLOCKED {coin} {side} (duelist {model} said {verdict}; …)` at WARNING; the `shadow_would_block` marker also lands in the `Trade result` line / `execute` event's `gate_results`, joinable to the ledger (coin/side/ts) and to the duel JSONL for free. Flip `shadow_mode` to `false` (one knob, hot-read per cycle) to make it block. Promotion check-in: once a VETO sample exists (grep the SHADOW lines), measure flagged-vs-outcome P/L — and consider tightening the escape bar, since an explicit rejection is stronger evidence than the old PASS abstention ever was.

### VETO verdict (2026-09-03)
`parse_verdict` accepts `"verdict": "VETO"` as a first-class token alongside LONG/SHORT/PASS/CLOSE: the model's ACTIVE rejection of a setup ("trading here is a bad idea"), categorically stronger than PASS. Routing: `route_verdict` treats VETO like PASS for the no-action path (shadow-forecast attach on the Trade result line) but it is **excluded from every structural-override hint** — whale/composite/breakout force-execute can never upgrade a VETO. `maybe_execute` carries an independent top-level guard returning `reason: ai_veto (...)` before the override section, closing the direct-call hole (server `/execute`, MCP) where downstream PASS equality-checks would otherwise let an unknown token fall through to the `side or "long"` default and blind-LONG. The primary's own VETO gates nothing directly (it simply never trades); its purpose is (a) honest duel data — the duelist's VETO is what `duelist_veto_gate` keys on, and (b) forcing the models to distinguish "not excited" from "actively dangerous", which PASS alone conflated.
